...
class Move:
    ...
    def __str__(self):
        return str(self.move) + " for utility " + str(self.utility)
...
class AlphaBetaAIv2:
    ...
    """
    def choose_move(self, board, moves=None):
        # TODO: Store moves in a priority queue sorted by utility
        # Recent counters
        self.num_calls = 0
        self.nodes_evaluated = 0

        first_iteration = False
        # If no ideal move order is known
        if moves is None:
        ...
        if first_iteration:
            for move in moves:
                ...
                curr_move = Move(move, curr_utility)

                heappush(pqueue, curr_move)
